...
function createQuestionCard(data) {
  ...
  div.innerHTML = `
    ...
    <div class="options-box" style="${data.type === "text" ? "display:none" : ""}">
      <input class="opt1" value="${data.options?.[0] || ""}" placeholder="اختيار 1" />
      <input class="opt2" value="${data.options?.[1] || ""}" placeholder="اختيار 2" />
      <input class="opt3" value="${data.options?.[2] || ""}" placeholder="اختيار 3" />
      <input class="opt4" value="${data.options?.[3] || ""}" placeholder="اختيار 4" />
    </div>

    <input class="correct" value="${data.correct || ""}" placeholder="الإجابة الصح" />

    <textarea class="hint" placeholder="💡 التلميح">${data.hint || ""}</textarea>
