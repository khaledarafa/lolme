...
function rebuildLayersUI(){
  ...
  $$(".select-layer").forEach(b=>b.addEventListener("click",e=>{ selectedIndex=+e.currentTarget.dataset.idx; draw(); }));
  $$(".layer-text").forEach(inp=>inp.addEventListener("input",e=>{ layers[+e.currentTarget.dataset.idx].text=e.target.value; draw(); saveState(); }));
  $$(".layer-size").forEach(inp=>inp.addEventListener("input",e=>{ layers[+e.currentTarget.dataset.idx].fontSize=+e.target.value; draw(); saveState(); }));
  $$(".layer-color").forEach(inp=>inp.addEventListener("input",e=>{ layers[+e.currentTarget.dataset.idx].color=e.target.value; draw(); saveState(); }));
  $$(".layer-strokecolor").forEach(inp=>inp.addEventListener("input",e=>{ layers[+e.currentTarget.dataset.idx].strokeColor=e.target.value; draw(); saveState(); }));
  $$(".del-layer").forEach(b=>b.addEventListener("click",e=>{ layers.splice(+e.currentTarget.dataset.idx,1); selectedIndex=null; rebuildLayersUI(); draw(); saveState(); }));
}
...
// SAVE & LOAD STATE
function saveState(){ if(!img) return; localStorage.setItem("memeState",JSON.stringify({imgSrc:img.src,layers})); }
function loadState(){ 
  const stateStr = localStorage.getItem("memeState"); 
  if(!stateStr) return;
  try{ const state=JSON.parse(stateStr); 
...
// EVENTS
$("#darkOverlay").addEventListener("change",(e)=>{ applyOverlay=e.target.checked; draw(); });
$("#blurBackground").addEventListener("change",(e)=>{ applyBlur=e.target.checked; draw(); });
